...
# The async_log_request function returns two other function for later logging
...
# Mona.
response_logger, exception_logger = mona_logger.async_log_request(
...
try:
    ...
    asyncio.run(response_logger(response.json()))
    print(response.json()["choices"][0]["text"])

except Exception:
    # Log exception to Mona
    asyncio.run(exception_logger())
