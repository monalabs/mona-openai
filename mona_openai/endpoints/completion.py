...
class CompletionWrapping(OpenAIEndpointWrappingLogic):
    ...
    def _get_full_privacy_analysis(self, prompt_privacy_analyzer, answers_privacy_analyzers):
        return {
            "prompt_phone_number_count": prompt_privacy_analyzer.get_phone_numbers_count(),
            "answer_unknown_phone_number_count": tuple(
                answer_analyzer.get_previously_unseen_phone_numbers_count((prompt_privacy_analyzer,)) for answer_analyzer in answers_privacy_analyzers
            ),
            "prompt_email_count": prompt_privacy_analyzer.get_emails_count(),
            "answer_unkown_email_count": tuple(
                answer_analyzer.get_previously_unseen_emails_count((prompt_privacy_analyzer,)) for answer_analyzer in answers_privacy_analyzers
            ),
        }

    @_get_texts
    @_get_analyzers(get_textual_analyzers)
    def _get_full_textual_analysis(self, prompt_textual_analyzer, answers_textual_analyzers):
        return {
            ...
            "answer_words_not_in_prompt_count": tuple(analyzer.get_words_not_in_others_count((prompt_textual_analyzer,)) for analyzer in answers_textual_analyzers),
            "answer_words_not_in_prompt_ratio": tuple(analyzer.get_words_not_in_others_count((prompt_textual_analyzer,)) / analyzer.get_word_count() for analyzer in answers_textual_analyzers),
        }
